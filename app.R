...
tmp1 <- readRDS("./data/LSOA/census_2021_LSOA_data.rds") #LSOA level data (merged)
tmp2 <- readRDS("./data/LTLA/census_2021_LTLA_data.rds") #LTLA level data (merged)
tmp3 <- unique(tmp2$local_authority) #list of UK Local Authority's 2021
tmp4 <- unique(tmp2$type) #list of data types present
tmp5 <- tmp2 %>% select(type,variable) %>% distinct(.) #list of distinct entries
...
server <- function(input, output, session) {
    ...
    # Update choices for SelectizeInput 2 based on the input of SelectizeInput 2 
    observe({
        ...
        # Update selectizeInput 2 with new choices
    ...
    # If accommodation is chosen, central heating systems are show, and vis vias.
    output$p3_p1 <- renderPlot({
        ...
        tryCatch({
            ...
            #suppress errors when no filtering value is present
            return(NULL)
        })
